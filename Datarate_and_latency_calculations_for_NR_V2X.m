% Currently these results are based on the PDSCH performmance with OFDM which uses simialr 
...
t_tx_NR = ceil(n_RB_NR/n_RB_slot_NR).*(t_slot*1e3); %
...
for i=1:length(snr_d)
    ...
    ICI_plus_noise = (ICI + pn)/sigma_sym;
      ...
      estimat = reshape(channel,[length(channel(:,1)),length(channel(1,:))/10,10]); % 1000 channel realizations
      sigma_mmse(:,:)= mean(((abs(estimat).^2)./((abs(estimat).^2)+ ICI_plus_noise)).^2,2);
    ...
    for ind =1:mcs_len
        ...
        snr_ieesm=10.*log10((beta(ind)/2).*(lambertw(exp(1).*((sum(exp(-snr_real'./beta(ind))./sqrt(((2.*snr_real')./beta(ind)) + 1))./10).^(-2))) - 1));  % effective SINR calculations
    ...
    latency_nr(i)= mean(min(latency_temp));  % Select the MCS provide Minimum latency
end
