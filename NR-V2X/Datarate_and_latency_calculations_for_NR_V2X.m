...
for i=1:length(snr_d)
    ...
    for ind =1:mcs_len
        ...
        snr_ieesm=10*log10((beta(ind)/2).*(lambertw(exp(1).*(mean(exp(-snr_real(:,1:(12*n_RB_NR(ind)),:)./beta(ind))./sqrt(((2.*snr_real(:,1:(12*n_RB_NR(ind)),:))./beta(ind)) + 1),[2 3]).^(-2))) - 1)); %Effective SINR mapping for used number of Resource blocks
